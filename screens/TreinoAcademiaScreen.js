...
import TreinosAcademiaIniciante from '../data/TreinosAcademiaIniciante';
import TreinosAcademiaIntermediario from '../data/TreinosAcademiaIntermediario';
import TreinosAcademiaAvancado from '../data/TreinosAcademiaAvancado';
...
const getGrupoMuscular = (nivel, dia) => {
  const gruposPorNivel = {
    iniciante: {
      segunda: 'Glúteos',
      quarta: 'Bíceps e Abdômen',
      sexta: 'Posterior de Coxa',
    },
    intermediario: {
      segunda: 'Glúteos',
      terça: 'Costas, Ombros e Bíceps',
      quarta: 'Quadríceps',
      quinta: 'Abdômen',
      sexta: 'Posterior de Coxa',
    },
  ...
  return gruposPorNivel[nivel]?.[dia] || '';
};

export default function TreinoAcademiaScreen({ nivel, onVoltar }) {
  const getTreinosPorNivel = () => {
    if (nivel === 'intermediario') return TreinosAcademiaIntermediario;
    if (nivel === 'avancado') return TreinosAcademiaAvancado;
    return TreinosAcademiaIniciante;
  };

  const treinos = getTreinosPorNivel();

  const diasComTreino = Object.keys(treinos).filter((dia) => {
    const dadosDia = treinos[dia];
    return (
      (Array.isArray(dadosDia?.exercicios) && dadosDia.exercicios.length > 0) ||
      (Array.isArray(dadosDia) && dadosDia.length > 0)
    );
  });

  const [diaSelecionado, setDiaSelecionado] = useState(diasComTreino[0]);
  ...
  const exerciciosDia =
    treinos[diaSelecionado]?.exercicios || treinos[diaSelecionado] || [];
      ...
      <View style={styles.wrapper}>
        ...
        >
          <Text style={styles.dropdownTexto}>
            {nomeFormatado[diaSelecionado]} - {getGrupoMuscular(nivel, diaSelecionado)}
          </Text>
        ...
        {dropdownAberto && (
          <View style={styles.dropdownLista}>
            {diasComTreino.map((dia) => (
              ...
              >
                <Text style={styles.dropdownTexto}>
                  {nomeFormatado[dia]} - {getGrupoMuscular(nivel, dia)}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
        ...
        {exerciciosDia.map((ex, index) => {
          ...
          return (
            <View key={index} style={styles.card}>
              ...
              <Text style={styles.texto}>Séries: {ex.series} | Repetições: {ex.repeticoes}</Text>
